...
import 'package:Ghorbeh/mainScreens/shepping_details.dart';
...
class CartPage extends StatelessWidget {
  ...
  double calculateTotalPrice(List<CartItem> cartItems) {
    ...
    double totalPrice = 0.0;
    int isInOffer5Books = 0;
    int isInOffer3book = 0;


    if (cartItems.length == 3) {
      for (int i = 0; i < 3;++i) {
        if (cartItems[i].price <= 5.0) {
          isInOffer3book=isInOffer3book+1;
        }
      }
      if (isInOffer3book==cartItems.length) {
        totalPrice = 10.0;
      }
    }else
    if (cartItems.length == 5) {
      for (int i = 0; i <5;++i) {
        if (cartItems[i].price <= 5.0) {
          isInOffer5Books=isInOffer5Books+1;
        }
      }
      if (isInOffer5Books==cartItems.length) {
        totalPrice = 15.0;
      }
    }else
